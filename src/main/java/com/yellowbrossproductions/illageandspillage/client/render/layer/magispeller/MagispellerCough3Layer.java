...
public class MagispellerCough3Layer<T extends LivingEntity> extends RenderLayer<T, MagispellerModel<T>> {
    ...
    public MagispellerCough3Layer(RenderLayerParent<T, MagispellerModel<T>> p_i226039_1_, EntityModelSet crap) {
        super(p_i226039_1_);
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entitylivingbaseIn instanceof MagispellerEntity && ((MagispellerEntity) entitylivingbaseIn).getGlowState() == 9 && !entitylivingbaseIn.isInvisible()) {
