...
public class MagispellerDispenserLayer<T extends LivingEntity> extends RenderLayer<T, MagispellerModel<T>> {
    ...
    public MagispellerDispenserLayer(RenderLayerParent<T, MagispellerModel<T>> p_i226039_1_, EntityModelSet crap) {
        super(p_i226039_1_);
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entitylivingbaseIn instanceof MagispellerEntity && ((MagispellerEntity) entitylivingbaseIn).getGlowState() == 5 && !entitylivingbaseIn.isInvisible()) {
