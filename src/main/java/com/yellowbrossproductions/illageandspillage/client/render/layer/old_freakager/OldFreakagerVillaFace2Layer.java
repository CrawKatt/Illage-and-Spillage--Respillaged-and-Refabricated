...
public class OldFreakagerVillaFace2Layer<T extends LivingEntity> extends RenderLayer<T, OldFreakagerModel<T>> {
    private static final ResourceLocation TEXTURE = new ResourceLocation("illageandspillage", "textures/entity/freakager/old/intro2.png");
    private final EntityModel<T> freakamodel;

    public OldFreakagerVillaFace2Layer(RenderLayerParent<T, OldFreakagerModel<T>> p_i226039_1_, EntityModelSet crap) {
        super(p_i226039_1_);
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entitylivingbaseIn instanceof OldFreakagerEntity && ((OldFreakagerEntity) entitylivingbaseIn).getVillagerFace() == 2 && !entitylivingbaseIn.isInvisible()) {
